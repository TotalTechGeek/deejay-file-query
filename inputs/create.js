...
import { xlsxObservable } from './xlsx.js'
 ...
 * @param {string} file
 * @param {'csv'|'bigjson'|'json'|'avro'|'xml'|'parquet'|'xlsx'|'custom'|'none'} format
...
export function createInput(file, format, additional) {
  if (!format) {
    ...
    if (extension !== 'csv' && extension !== 'json' && extension !== 'avro' && extension !== 'xml' && extension !== 'parquet' && extension !== 'xlsx') {
  ...
  if (format === 'xlsx') {
    return xlsxObservable(file, additional)
  }
...
export function addInputs(engine) {
  ...
  // @ts-ignore
  engine.addMethod('xlsx', data => xlsxObservable(...[].concat(data)))

  engine.addMethod('rx.interval', ([amount]) => interval(amount))
